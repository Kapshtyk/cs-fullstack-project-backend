...
using static Ecommerce.Services.CartItemService.DTO.CreateCartItemDto;
...
namespace Ecommerce.Controllers
{
    ...
    public class CartItemController(ICartItemService cartItemService, IAuthorizationService authorizationService, ICartItemRepo cartItemRepo) : AppController<CartItem, CartItemFilterOptions, GetCartItemDto, PartialUpdateCartItemDto, CreateCartItemDto, UpdateCartItemDto>(cartItemService)
    {
        ...
        public override async Task<ActionResult<PaginatedResult<CartItem, GetCartItemDto>>> GetItems([FromQuery] CartItemFilterOptions filteringOptions)
        {
            ...
            if (claimId?.Value != filteringOptions.UserId.ToString() && claimRole?.Value != "Admin")
            {
                throw new UnauthorizedAccessException("You are not allowed to view cart items for another user");
            }
        ...
        public override async Task<ActionResult<GetCartItemDto>> GetItem(int id)
        {
        ...
        public override async Task<ActionResult<GetCartItemDto>> CreateItem([FromBody] CreateCartItemDto dto)
        {
            var claimId = HttpContext.User.FindFirst(c => c.Type == ClaimTypes.NameIdentifier);
            var claimRole = HttpContext.User.FindFirst(c => c.Type == ClaimTypes.Role);
        ...
        [SwaggerResponse(403, "User is not the owner of the cart item or not an admin", typeof(ProblemDetails))]
        [SwaggerResponse(404, "The cart item was not found", typeof(ProblemDetails))]
        public override async Task<ActionResult<GetCartItemDto>> PartialUpdateItem([SwaggerParameter(description: "Cart item id")] int id, [FromBody] PartialUpdateCartItemDto dto)
        {
            ...
            var updatedEntity = await _cartItemService.UpdateAsync(dto, id);
        ...
        [HttpPut("{id}")]
        [SwaggerIgnore]
        public override Task<ActionResult<GetCartItemDto>> UpdateItem(int id, [FromBody] UpdateCartItemDto dto)
        {
