...
namespace Ecommerce.Infrastructure.DatabaseFunctions
{
    ...
    public partial class CreateOrderFromCart : Migration
    {
        ...
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(@"
                CREATE OR REPLACE FUNCTION public.create_order_from_cart(
                    p_user_id integer)
                    RETURNS TABLE(order_id integer, user_id integer, order_date timestamp with time zone) 
                    LANGUAGE 'plpgsql'
                    COST 100
        ...
        {
            migrationBuilder.Sql("DROP FUNCTION public.create_order_from_cart(integer)");
        }
    }
}
